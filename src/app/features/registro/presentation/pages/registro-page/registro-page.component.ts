...
interface StepPresentation extends RegistroStep { description: string; }

@Component({
  selector: 'app-registro-page',
  standalone: true,
  imports: [ReactiveFormsModule, InstitutionalHeaderComponent, SidebarComponent, SectionHeaderComponent, HelpPanelComponent, StepNavigationComponent, IdentificacionFormComponent, OrigenResidenciaFormComponent, ContactoFormComponent, FotografiaFormComponent],
...
export class RegistroPageComponent implements OnInit {
  ...
  readonly form = this.fb.group({
    ...
    nombres: ['', Validators.required], primerApellido: ['', Validators.required], segundoApellido: [''],
    fechaNacimiento: ['', Validators.required], sexo: ['', Validators.required], identidadGenero: [''], nacionalidad: ['', Validators.required],
    entidadNacimiento: ['', Validators.required], municipioNacimiento: ['', Validators.required], estadoCivil: ['', Validators.required], vulnerabilidad: [''], dependientes: [''],
    entidadResidencia: ['', Validators.required], municipioResidencia: ['', Validators.required], calle: ['', Validators.required], numeroExterior: [''], numeroInterior: [''], colonia: [''], codigoPostal: ['', [Validators.required, Validators.pattern(/^\d{5}$/)]],
    telefono: [''], correo: ['', Validators.email], nombreContactoEmergencia: ['', Validators.required], contactoEmergencia: ['', Validators.required],
    fechaToma: ['', Validators.required], biometrico: ['si' as 'si' | 'no', Validators.required], identificadorReferencia: ['CURP-BIO-9923847-X', Validators.required],
  ...
  readonly steps: StepPresentation[] = [
    { key:'identificacion', title:'Identificación básica', requiredCount:9, icon:ASSETS.icons.identification, description:'Información general, contacto y escolaridad inicial del elemento policial.' },
    { key:'origen', title:'Origen y residencia', requiredCount:4, icon:ASSETS.icons.origin, description:'Información de residencia y domicilio actual del elemento policial.' },
    { key:'contacto', title:'Contacto', requiredCount:2, icon:ASSETS.icons.contact, description:'Información de contacto y referencia para casos de emergencia.' },
    { key:'fotografia', title:'Fotografía', requiredCount:1, icon:ASSETS.icons.camera, description:'Registrar o consultar fotografía actualizada dentro del módulo Datos Personales.' }
  ];
  ...
  readonly generalProgress = computed(() => Math.round(this.sectionProgress().reduce((sum, item) => sum + item.percentage, 0) / this.sectionProgress().length));
  ...
  setStep(step: RegistroStepKey): void { this.facade.setStep(step); window.scrollTo({ top:0, behavior:'smooth' }); }
  previous(): void { const i=this.activeIndex(); if(i>0) this.setStep(this.steps[i-1].key); }
  next(): void { const i=this.activeIndex(); if(i<this.steps.length-1) this.setStep(this.steps[i+1].key); }
  async saveDraft(): Promise<void> { await this.facade.saveDraft(this.form.getRawValue() as RegistroDraft); }
  toggleSidebar(): void { this.sidebarOpen.update(value => !value); }

  onPhotoSelected(file: File): void {
    this.photoError.set('');
    if (!['image/jpeg','image/png'].includes(file.type)) { this.photoError.set('Solo se permiten archivos JPG o PNG.'); return; }
    if (file.size > 2 * 1024 * 1024) { this.photoError.set('La fotografía debe pesar máximo 2 MB.'); return; }
    const reader = new FileReader();
    reader.onload = () => this.form.patchValue({ photoDataUrl: String(reader.result ?? ''), photoName: file.name });
    reader.readAsDataURL(file);
  }
  removePhoto(): void { this.form.patchValue({ photoDataUrl:'', photoName:'' }); this.photoError.set(''); }
